...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const hasCustomTasks = customTasks && Array.isArray(customTasks) && customTasks.length > 0;
      const hasScreenTime = screenTimeTarget !== undefined && screenTimeTarget !== null;
      
      if (!hasCustomTasks && !hasScreenTime) {
        return res.status(400).json({ message: 'At least one task or screen time target is required' });
      }
      ...
      if (task) {
        ...
        if (screenTimeTarget !== undefined) {
          task.screenTimeTarget = screenTimeTarget !== null ? screenTimeTarget : null;
        }
        task.updatedAt = new Date();
        await task.save();
      } else {
        // Create new task only if there's data to save
        task = new Task({
          userId: req.user._id,
          weekStartDate: weekStart,
          weekEndDate: weekEnd,
...
router.delete('/custom-task/:taskIndex', authenticateToken, async (req, res) => {
  try {
    const { taskIndex } = req.params;
    const weekStart = getWeekStartDate();
